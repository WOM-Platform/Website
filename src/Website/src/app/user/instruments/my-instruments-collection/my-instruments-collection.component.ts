...
export class MyInstrumentsCollectionComponent implements OnInit, OnDestroy {
    ...
    this.userService.userOwnershipStatus.subscribe({
      next: (res) => {
        if (res && res.sources) {
          ...
          this.uiInstruments = res.sources.map(
            (instrument: InstrumentEditing, index: number) => {
  ...
  updateInstrumentField(
    ...
    delete (updatedInstrument as any).isOpen;

    this.sourceService.update(updatedInstrument as any).subscribe({
      next: () => {
        (instrument as Record<string, any>)[key] = value;
  ...
  updateUIInstrument(instrumentId: string, key: string, value: any) {
    const instrumentUI = this.uiInstruments.find(
      (uiInstrument) => uiInstrument.id === instrumentId
    );
    if (instrumentUI) {
      (instrumentUI as Record<string, any>)[key] = value;
    }
  }

  findEditingInstrumentById(instrumentId: string) {
    return this.editingInstruments.find(
