...
import {
  ...
  Instrument,
  InstrumentEditing,
} from "src/app/_models/instrument";
import { AimsService, AuthService, UserService } from "src/app/_services";
...
export class DialogViewEditInstrumentComponent implements OnInit, OnDestroy {
  ...
  instrumentView: Instrument = new Instrument();

  currentUser!: UserMe;

  action: string;

  newApiKeySelector: string = "";
    ...
    return this.aimsService.getAll().pipe(
      switchMap((aimList) => {
        const aimsView = this.aimsService.findMatchingCodes(
          aimList,
          instrumentEditing.aims.enabled
        );
        return of(aimsView);
      })
    );
  ...
  updateInstrumentField(key: string, value: any, isTableToUpdate: boolean) {
    ...
    (updatedInstrument as Record<string, any>)[key] = value;
  ...
  fetchAccessList(): void {
    this.sourceService
      .getInstrumentAccessList(this.instrumentEditing.id)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  confirmAndDeleteAccess(access: any): void {
    ...
    dialogDeleteRef
      ...
      .pipe(
        switchMap((result) => {
          if (result) {
            return this.sourceService.deleteInstrumentAccess(
              this.instrumentEditing.id,
              access.userId
            );
          }
          return of(null);
  ...
  generateApiKey() {
    if (!this.newApiKeySelector || !this.instrumentView?.id) {
      return;
    }

    const sourceId = this.instrumentView.id;
    const selector = this.newApiKeySelector.trim();
    ...
    this.auth
      .createApiKey(sourceId, selector)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res: any) => {
          this.generatedApiKey = res.apiKey;
          this.snackBar.open("API key generated successfully", "OK", {
            duration: 3000,
          });
          this.cd.detectChanges();
        },
        error: (err) => {
          this.apiKeyError = err?.error?.title || "Failed to generate API key";
          this.snackBar.open(`⚠️ ${this.apiKeyError}`, "OK", {
            duration: 3000,
          });
        },
      });
  ...
  addAccessAndUpdateList(access: any): void {
    this.sourceService
      .addInstrumentAccess(this.instrumentEditing.id, access.access.id)
      .pipe(
        switchMap(() =>
          this.sourceService.getInstrumentAccessList(this.instrumentEditing.id)
        ),
        takeUntil(this.destroy$)
      )
      .subscribe({
