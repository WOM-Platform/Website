...
export class BillingCheckoutComponent implements OnInit {
  ...
  checkout(id: string) {
    ...
    this.billingService.checkout(data).subscribe(
      (result) => {
    ...
    switch (interval) {
      case "year":
        if (intervalCount === 1)
          ...
          return this.translationService.instant(
            "BILLING.RECURRING.INTERVAL.YEARS"
          );
      case "month":
        if (intervalCount === 1)
          return this.translationService.instant(
          ...
          return this.translationService.instant(
            "BILLING.RECURRING.INTERVAL.MONTHS"
          );
      case "day":
        if (intervalCount === 1)
          return this.translationService.instant(
            "BILLING.RECURRING.INTERVAL.DAY"
          );
        else
          return this.translationService.instant(
            "BILLING.RECURRING.INTERVAL.DAYS"
          );
      default:
        return "N/A";
    }
