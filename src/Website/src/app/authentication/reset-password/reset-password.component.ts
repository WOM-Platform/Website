...
export class ResetPasswordComponent implements OnInit {
  ...
  resetPassword(): void {
    ...
    this.userService
      .passwordReset(this.userId, this.token, pswd)
      .pipe()
      .subscribe({
        next: () => {
          const snackTitle = this.translate.instant(
            "RESET_PASSWORD.SNACKBAR_SUCCESS"
          );
          const snackAction = this.translate.instant(
            "RESET_PASSWORD.SNACKBAR_HOME"
          );
  ...
  checkPasswords(): boolean {
    if (
      ...
      this.form.controls.passwordRepeat === null
    ) {
      return false;
    }
    return (
