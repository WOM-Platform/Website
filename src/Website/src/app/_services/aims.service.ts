...
export class AimsService {
  ...
  getByCode(code: string): Observable<Aim> {
    const cachedAims = this.storageService.get("aimsData") as Aim[];
    if (cachedAims) {
      const aim = cachedAims.find((a: Aim) => a.code === code);
      return of(aim as Aim);
    } else {
      return this.getAll().pipe(
        map((aims) => {
          const aim = aims.find((a: Aim) => a.code === code);
          return aim as Aim;
        })
      );
    }
