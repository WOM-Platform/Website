...
export class PosService {
  ...
  update(pos: Pos): Observable<Pos> {
    return this.http
      .put<Pos>(this.localUrlV1 + pos.id, {
        name: pos.name,
    ...
    return this.http.get(`${this.localUrlV1}${posId}/offers`, {}).pipe(
      switchMap((offers: any[]) => {
        ...
        const offerDetailsRequests = offers.map((offer) =>
          this.http
            .get(`${environment.baseUrl}render/offer/${offer.id}?style=other`, {
              responseType: "blob",
            })
            .pipe(
  ...
  editOfferTitle(
    ...
    return this.http.put(
      `${this.localUrlV1}${posId}/offers/${offerId}/title`,
      offerEdited
    );
  }
  ...
  getOfferQrCode(offer: Offer): Observable<any> {
    return this.http
      .get(`https://dev.wom.social/api/render/offer/${offer.id}`, {
        responseType: "blob",
      })
      .pipe(
        map((offerDetails: any) => {
  ...
  updateOfferStatus(posId: string, offerId: string, deactivatedValue: boolean) {
    return this.http.put(
      `${this.localUrlV1}${posId}/offers/${offerId}/deactivation`,
  ...
  uploadFile(posId: string, file: File): Observable<HttpEvent<any>> {
    ...
    return this.http
      .post<any>(`${this.localUrlV1}${posId}/cover`, formData, {
        headers: headers,
        reportProgress: true,
        observe: "events",
      })
      .pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          ...
          return throwError(
            () => new Error("File upload failed. Please try again later.")
          );
        })
      );
  }
